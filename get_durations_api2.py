...
for url in urls:
    try:
        ...
        # Oembed doesn't have duration. We will try fetching the page and parsing ytInitialPlayerResponse.
        ...
        start_idx = html.find("ytInitialPlayerResponse = ")
        if start_idx != -1:
            start_idx += len("ytInitialPlayerResponse = ")
            end_idx = html.find("};", start_idx) + 1
            if end_idx != 0:
                json_str = html[start_idx:end_idx]
                try:
                    data = json.loads(json_str)
                    duration_s = int(data.get("videoDetails", {}).get("lengthSeconds", "0"))
                    total_duration += duration_s
                    print(f"Parsed {video_id}: {duration_s}s")
                except json.JSONDecodeError as je:
                    print(f"JSON error for {video_id}", file=sys.stderr)
            else:
                 print(f"End not found for {video_id}", file=sys.stderr)
        else:
             print(f"ytInitialPlayerResponse not found for {video_id}", file=sys.stderr)
